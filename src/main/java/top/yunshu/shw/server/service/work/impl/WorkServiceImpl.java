...
public class WorkServiceImpl implements WorkService {
    ...
    public List<Work> getStudentUnDoneWork(String studentId) {
        return studentGroupDao.findGroupIdByStudentNumber(studentId).parallelStream().map(workDao::findAllByGroupId).flatMap(Collection::stream).filter(work -> !uploadDao.existsByWorkId(work.getId())).collect(Collectors.toList());
    }
